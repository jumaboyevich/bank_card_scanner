...
final class ImageAnalyzer {
    ...
    lazy var requestHandler: ((VNRequest, Error?) -> Void)? = { [weak self] request, _ in
        ...
        for result in results {
            ...
            } else if let month1 = month.captures(in: string).last.flatMap(Int.init),
                // Appending 20 to year is necessary to get correct century
                let year1 = year.captures(in: string).last.flatMap({ Int($0) }) {
                creditCard.expireDate = DateComponents(year: year1, month: month1)
